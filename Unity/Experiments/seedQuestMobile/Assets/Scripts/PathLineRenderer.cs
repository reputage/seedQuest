...
public class PathLineRenderer : MonoBehaviour {
    ...
    private void setLineVertices() {
        ...
        Vector2[] normals = new Vector2[path.Length * 2];
        ...
        for (int i = 0; i < path.Length - 1; i++) {
            ...
            normals[i] = path[i] - pVec * lineWidth / 2;
            normals[i + path.Length] = path[i] + pVec * lineWidth / 2;
        } 
        ...
        normals[path.Length - 1]  = path[path.Length - 1] - pt * lineWidth / 2;
        normals[normals.Length - 1] = path[path.Length - 1] + pt * lineWidth / 2;

        for (int i = 0; i < normals.Length; i++)
            vertices[i] = new Vector3(normals[i].x, normals[i].y);
